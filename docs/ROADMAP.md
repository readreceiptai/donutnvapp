...
- **#70 Google Maps key rejects the native WebView origin** — analysed, fix is a Cloud Console change (Kevin's thread). Key `AIzaSyDB_R2iqjeyLH3JIPO3_EW-aQhS216WoxM` (`VITE_GOOGLE_MAPS_API_KEY`, baked into every bundle incl. native), HTTP-referrer-restricted (confirmed via REQUEST_DENIED on server-side use). Native origins to allow: **`capacitor://localhost/*`** (iOS, observed live) and **`https://localhost/*`** (Android, Capacitor default). Recommendation: a SEPARATE Maps-JS-only key for native, not widening the web key — see PROXIMITY-PUSH.md. Verify in Simulator once the key exists.
