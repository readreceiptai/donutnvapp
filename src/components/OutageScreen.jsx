// Branded "we'll be right back" outage screen (from status-pages/outage.html).
// Shared by the top-level ErrorBoundary (render crashes) and BackendHealthGate
// (Supabase unreachable). If onRetry is given, the button runs it (an immediate
// re-check); otherwise it reloads the page.
export default function OutageScreen({ onRetry, checking = false }) {
  const handle = onRetry ? onRetry : () => window.location.reload()
  return (
    <div style={S.wrap}>
      <main style={S.card} role="alert" aria-live="polite">
        ...
        <button style={S.btn} onClick={handle} disabled={checking}>{checking ? 'Checking…' : 'Try again'}</button>
