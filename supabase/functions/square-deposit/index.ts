// ── Square event deposit — Supabase Edge Function (CORS-fixed) ──
...
Deno.serve(async (req) => {
  ...
  if (b.deposit_status === 'requested' && b.deposit_url) return json({ ok: true, alreadyRequested: true, url: b.deposit_url })
  if (b.deposit_status === 'paid') return json({ ok: true, alreadyPaid: true })

  const { data: tenant } = await supabase.from('tenants').select('square_location_id, name').eq('id', b.tenant_id).maybeSingle()
  ...
  const linkRes = await fetch(`${SQUARE_BASE}/v2/online-checkout/payment-links`, {
    ...
    body: JSON.stringify({
      ...
      quick_pay: { name: `DonutNV deposit — ${b.event_type || 'event'}${b.event_date ? ` ${b.event_date}` : ''}`, price_money: { amount, currency: 'USD' }, location_id: locationId },
      checkout_options: { redirect_url: `${appBase}/track/${b.tracking_token}` },
      pre_populated_data: { buyer_email: b.contact_email || undefined, buyer_phone_number: b.contact_phone || undefined },
      payment_note: `DonutNV booking ${booking_id}`,
    }),
  })
  const linkJson = await linkRes.json().catch(() => ({}))
  if (!linkRes.ok) return json({ error: 'Square rejected the request', detail: linkJson?.errors ?? linkJson }, 502)
  ...
  await supabase.from('bookings').update({
    deposit_amount_cents: amount, deposit_status: 'requested', deposit_url: url,
    deposit_link_id: link.id ?? null, square_order_id: link.order_id ?? null,
  ...
  if (ghlToken && ghlLocation && b.ghl_contact_id) {
    await fetch(`https://services.leadconnectorhq.com/contacts/${b.ghl_contact_id}`, {
      ...
      body: JSON.stringify({ customFields: [{ key: 'deposit_link', field_value: url }, { key: 'deposit_amount', field_value: (amount / 100).toFixed(2) }], tags: ['deposit-requested'] }),
    }).catch(() => {})
  }
